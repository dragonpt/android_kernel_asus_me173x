...
//#define COMPAL_LOG_ENABLE
#ifdef COMPAL_LOG_ENABLE
#define COMPAL_LOG(fmt, ...) printk(KERN_INFO "compal_touch:" fmt, ##__VA_ARGS__)
#else
#define COMPAL_LOG(fmt, ...)
#endif

#define COMPAL_MTK_TOUCH
...
static unsigned short f11_cmd_base_addr;

#ifdef COMPAL_MTK_TOUCH
#define DRIVER_NAME "compal-touch"
...
static int compal_clear_interrupts(struct synaptics_rmi4_data *rmi4_data);
...
static const struct i2c_device_id synaptics_rmi4_id_table[] = {{"compal-tpd", 0},{}};
unsigned short force[] = {0, 0x20, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short * const forces[] = { force, NULL };
static struct i2c_board_info __initdata i2c_tpd={ I2C_BOARD_INFO("compal-tpd", (0x20))};//0x20<<1=0x40 or 0x70<<1=0xE0

static struct i2c_driver tpd_i2c_driver = {
	.probe = synaptics_rmi4_probe,
	.remove = __devexit_p(synaptics_rmi4_remove),
	.detect = tpd_detect,
	.driver.name = "compal-tpd",
...
static int compal_synaptics_rmi4_irq_enable(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	int retval=0;
	if (enable) {
		retval = compal_clear_interrupts(rmi4_data);
		if (retval < 0)
			return retval;
...
static int compal_synaptics_rmi4_irq_acquire(struct synaptics_rmi4_data *rmi4_data, bool enable)
{
	int retval = 0;
	unsigned char intr_status;
...
static void tpd_eint_handler(void)
{
	...
	TPD_DEBUG_PRINT_INT;
	printk("Zara: interrupt has been triggered\n");
	tpd_flag=1;
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	printk("Zara: complete input_sync\n");
...
{
#ifdef COMPAL_MTK_TOUCH
	return compal_synaptics_rmi4_irq_enable(rmi4_data, true);
#endif
}
...
{
#ifdef COMPAL_MTK_TOUCH
	return compal_synaptics_rmi4_irq_acquire(rmi4_data, true);
#endif
}
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F11:
				...
				f11_cmd_base_addr = fhandler->full_addr.cmd_base;
		...
		const struct i2c_device_id *dev_id)
{
	...
	rmi4_data->input_dev->name = "compal-tpd";
	...
	gThread = kthread_run(touch_event_handler, 0, "compal-tpd");
	if ( IS_ERR(gThread) ) {
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (mIsEnableGestureWakeUp)
		device_ctrl = (device_ctrl | NO_SLEEP_OFF | SPECIAL_SLEEP);
	else
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	COMPAL_LOG("device_ctrl=%d", device_ctrl&0x03);
	//if ( (device_ctrl & MASK_2BIT) == SPECIAL_SLEEP ) {
	if ( mIsEnableGestureWakeUp ) {
		COMPAL_LOG("calibration from speical mode");
		retval = synaptics_rmi4_i2c_write(rmi4_data,
			f11_cmd_base_addr,
			&rezero,
			sizeof(rezero));
...
static int compal_local_init(void)
{
...
static struct tpd_driver_t synaptics_rmi4_driver = {
	...
	.tpd_local_init = compal_local_init,
	.suspend = synaptics_rmi4_suspend,
	.resume = synaptics_rmi4_resume,
};
