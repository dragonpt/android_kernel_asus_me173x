...
{                                          
	   ...
	   OV7675_write_cmos_sensor(0x1e,0x37);//mirror and flip   //MTK original setting is 0x07. Adjust to 0x37 to avoid abnormal front cam orientation on TP, 3rd party App
...
static void OV7675_HVMirror(ACDK_SENSOR_IMAGE_MIRROR_ENUM SensorImageMirror)
{
	...
	{
		case IMAGE_NORMAL:
		while( (temp_reg2 != (0x30|temp_reg1)) && retry >= 0 ){
			...
			break;
		case IMAGE_H_MIRROR:
			while( (temp_reg2 != (0x10|temp_reg1)) && retry >= 0 ){
			...
			break;
		case IMAGE_V_MIRROR:
			while( (temp_reg2 != (0x20|temp_reg1)) && retry >= 0  ){
		...
		case IMAGE_HV_MIRROR:
			while( (temp_reg2 != (0x00|temp_reg1)) && retry >= 0  ) {
...
static void OV7675_HVMirror__MTK_ORIGINAL(ACDK_SENSOR_IMAGE_MIRROR_ENUM SensorImageMirror)
{
	volatile kal_uint32 temp_reg2=OV7675_read_cmos_sensor(0x1E), temp_reg1=(temp_reg2&0x0F);
	kal_uint32 iTemp;
        int retry = 3; 
