...
#define GPIO_DISP_LED_SIGNAL_EN      GPIO126
#define GPIO_DISP_VLED_EN            GPIO142   //3.3v
#define GPIO_DISP_LED_BL_EN          GPIO130   
// ---------------------------------------------------------------------------
//  Local Constants
// ---------------------------------------------------------------------------
...
static void lcm_mt8193_set_lvds_digital(void)
{
	...
	MT8193_REG_WRITE(LVDS_FMT_CTRL, RG_6BIT_FORMAT);//for 18bit LCM
}
...
#else
	MT8193_REG_WRITE(LVDS_OUTPUT_CTRL, (RG_LVDSRX_FIFO_EN | RG_SYNC_TRIG_MODE | RG_LVDS_E));
#endif
}
...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dpi.mipi_pll_clk_div2 = 0x8009E276;
...
{
#ifdef BUILD_LK
	printf("compal---------[LK/LCM] lcm_suspend() enter\n");

	lcm_set_gpio_output(GPIO_DISP_LED_SIGNAL_EN, 0);//BL enable
	MDELAY(1);
	lcm_set_gpio_output(GPIO_DISP_LED_BL_EN, 0);//BL power enable
	MDELAY(200);
	lcm_mt8193_disable_output();
	MDELAY(1);	
	lcm_set_gpio_output(GPIO_DISP_VLED_EN, 0);//3.3v	
#elif (defined BUILD_UBOOT)
		// do nothing in uboot
#else
    printk("compal----------[LCM] lcm_suspend() enter\n");
...
static void lcm_resume(void)
{
        ...
        MDELAY(1);
        lcm_set_gpio_output(GPIO_DISP_VLED_EN, 1);//3.3v
        MT8193_REG_WRITE(LVDS_OUTPUT_CTRL, (RG_LVDSRX_FIFO_EN | RG_SYNC_TRIG_MODE | RG_OUT_FIFO_EN | RG_LVDS_E));  //tigger LVDS Output
        MDELAY(200);
        lcm_set_gpio_output(GPIO_DISP_LED_BL_EN, 1);//BL power enable
        MDELAY(1);
        lcm_set_gpio_output(GPIO_DISP_LED_SIGNAL_EN, 1);        //BL enable
#elif (defined BUILD_UBOOT)
        // do nothing in uboot
#else
