...
struct delayed_work smart_charge_w;
...
MODULE_INFO Module_Table[] = {
	{0, 0, 1000, "/sys/devices/virtual/thermal/thermal_zone3/temp"}, // thermal
	{0, 0, 1000, "/sys/devices/virtual/thermal/thermal_zone2/temp"}, // battery
	{0, 0, 1, "/sys/class/power_supply/battery/bq24196_temp"}, // charger(bq24196)
};
...
#ifdef COMPAL_SMART_CHARGING_SUPPORT
#define MY_WORK_QUEUE_NAME "SMC"
static struct workqueue_struct *sm_queue;
#endif
...
#else
    bq24196_set_vreg(0x2A); //VREG 4.176V
#endif    
...
//	    if(upmu_get_cid() == 0x1020)
    ...
    if (BMT_status.bat_charging_state == CHR_ERROR) {
	if ((BMT_status.temperature >= 3) && (BMT_status.temperature <= 45)) {
		printk ("[BATTERY:bq24196] Battery Temperature Between 3 and 45, Start Charging\n");
...
void BAT_thread_bq24196(void)
{
    ...
    if(boot_check_once==1)
    {
        if( upmu_is_chr_det() == KAL_TRUE )
        {
            ...
            if(ret_val > 4110 && get_rtc_spare_fg_value() == 100)
            {
                g_bat_full_user_view = KAL_TRUE;                
            }
    ...
    if( upmu_is_chr_det_usb() == KAL_TRUE )
    {
	    ...
	    {
		g_first_check = 0;
		xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "[BAT_thread:bq24196] Keep Detecting charger type for NONSTANDARD_CHARGER !\r\n");
	    }
...
int Thermal_TempToLevel(int currentT) {
	...
	if (diff > 0)	{
		...
		else if (currentT >= 62 && currentT < 68)
		...
		if (currentT <= 58)
			Module_Table[THERMAL_MODULE].prelevel = 1;
		else if (currentT > 58 && currentT <= 64)
			Module_Table[THERMAL_MODULE].prelevel = 2;
		else if (currentT > 64)
			Module_Table[THERMAL_MODULE].prelevel = 3;
		string = "down";
...
int Charger_TempToLevel(int currentT) {
	...
	if (diff > 0)   {
		...
		else if (currentT >= 59 && currentT < 65)
		...
		if (currentT <= 55)
			Module_Table[CHARGER_MODULE].prelevel = 1;
		else if (currentT > 55 && currentT <= 61)
			Module_Table[CHARGER_MODULE].prelevel = 2;
		else if (currentT > 61)
			Module_Table[CHARGER_MODULE].prelevel = 3;
			string = "down";
...
void Smart_Charging(void) {
	...
	queue_delayed_work(workqueue, &smart_charge_w, msecs_to_jiffies(2000));
// Step1: Get currentT
...
// Step4: Set Ibattery to Charging IC
	...
	printk ("SMART_CHARGER : bq24196_get_chg_config()=%d, bq24196_get_ichg()=%d, gChargingDisabled=%d \n",
							bq24196_get_chg_config(), bq24196_get_ichg(), gChargingDisabled);
...
//    wake_lock_init(&battery_suspend_lock, WAKE_LOCK_SUSPEND, "battery wakelock");
    ...
    kthread_run(bat_thread_kthread, NULL, "bat_thread_kthread"); 
#ifdef COMPAL_SMART_CHARGING_SUPPORT
    workqueue =	create_singlethread_workqueue("smart charger");
    INIT_DELAYED_WORK(&smart_charge_w, Smart_Charging);
    queue_delayed_work(workqueue, &smart_charge_w, msecs_to_jiffies(2000));
#endif
