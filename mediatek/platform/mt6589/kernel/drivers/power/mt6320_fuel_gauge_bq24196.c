...
void fg_qmax_update_for_aging(void)
{
    if(g_HW_Charging_Done == 1) // charging full
    {
	    ...
	    if (gFG_BATT_CAPACITY_aging > gFG_BATT_CAPACITY)
            {
...
//4. update DOD0 after booting Xs
    if(gFG_booting_counter_I_FLAG == 1)
    {
        ...
        {      
			...
			{
				if( (( g_rtc_fg_soc != 0 					) &&					
					( (g_rtc_fg_soc-gFG_capacity_by_v) < 45 ) &&
					( gFG_capacity_by_v > 5 ))
                    ||
                    ( (g_rtc_fg_soc != 0) && 
			...
			{
				if( (( g_rtc_fg_soc != 0 					) &&					
					( (gFG_capacity_by_v-g_rtc_fg_soc) < 45 ) &&
					( gFG_capacity_by_v > 5 ))
                    ||
                    ( (g_rtc_fg_soc != 0) && 
                     (g_boot_reason == BR_WDT_BY_PASS_PWK || g_boot_reason == BR_WDT))
...
//    gFG_voltage = get_hw_ocv();
    ...
    gFG_BATT_CAPACITY_aging = (gFG_BATT_CAPACITY*94)/100;
    //FGADC_dump_register();        
