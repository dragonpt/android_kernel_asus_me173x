...
static int mtk_wdt_probe(struct platform_device *dev)
{
   ...
   udelay(100);
   printk("mtk_wdt_probe : done WDT_MODE(%x),MTK_WDT_REQ_MODE(%x),MTK_WDT_REQ_IRQ_EN(%x)\n",DRV_Reg32(MTK_WDT_MODE),DRV_Reg32(MTK_WDT_REQ_MODE),DRV_Reg32(MTK_WDT_REQ_IRQ_EN));
   return ret;
}
