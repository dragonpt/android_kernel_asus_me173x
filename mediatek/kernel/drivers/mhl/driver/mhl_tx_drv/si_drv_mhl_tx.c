...
extern int HalGpioGetTxIntPin(void);
...
#ifdef MDT_SUPPORT
		MHL_log_event(IRQ_RECEIVED, 0, HalGpioGetTxIntPin());
	//TX_DEBUG_PRINT(("g_state_for_mdt=%d,HalGpioGetTxIntPin11111111111111=%d#######################\n",g_state_for_mdt,HalGpioGetTxIntPin()));
	i=0;
    do
    {
...
#ifdef MDT_SUPPORT
	//MHL_log_event(ISR_FULL_END, 0, HalGpioGetTxIntPin());
	//MHL_log_event(IRQ_RECEIVED, 0, HalGpioGetTxIntPin());
#endif
//TX_DEBUG_PRINT(("g_state_for_mdt=%d,HalGpioGetTxIntPin555555555=%d#######################\n",g_state_for_mdt,HalGpioGetTxIntPin()));
...
bool_t SiiMhlTxDrvSendCbusCommand ( cbus_req_t *pReq  )
{
    ...
    uint8_t i, startbit;
	/*
	TX_DEBUG_PRINT(("mscAbortFlag=%d\n",mscAbortFlag));
	if (mscAbortFlag)
    {
        TX_DEBUG_PRINT(("Abort Timer is working in SiiMhlTxDrvSendCbusCommand!\n"));
        return false;
    }*/
    ...
    switch ( pReq->command )
    {
        ...
        case MHL_WRITE_BURST:
            ...
            else
            {
				...
				{
					SiiRegWrite((TX_PAGE_CBUS | 0x00C0) + i, *pData++ );
				}
	            startbit = 0x01 << 4;
			}
            break;
...
static void WriteInitialRegisterValues ( void )
{
	...
	SiiRegWrite(TX_PAGE_3 | 0x0018, 0x24);
