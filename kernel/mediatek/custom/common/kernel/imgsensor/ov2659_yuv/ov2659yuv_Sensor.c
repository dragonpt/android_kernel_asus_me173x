...
static void OV2659InitialSetting(void)
{
	...
	OV2659_write_cmos_sensor(0x4300,0x31);//YUYV //20130104 modify output fomart
	...
	*/
	...
	OV2659_write_cmos_sensor(0x3011,0xc2);//modify driving  4*
	//////
	spin_lock(&ov2659_drv_lock);
...
UINT32 OV2659GetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	pSensorResolution->SensorPreviewWidth= OV2659_IMAGE_SENSOR_SVGA_WIDTH - 2 * 10;
	pSensorResolution->SensorPreviewHeight= OV2659_IMAGE_SENSOR_SVGA_HEIGHT - 2 * 10;
	
	pSensorResolution->SensorFullWidth= OV2659_IMAGE_SENSOR_UVGA_WITDH - 2 * 10;  
	pSensorResolution->SensorFullHeight= OV2659_IMAGE_SENSOR_UVGA_HEIGHT - 2 * 10;
	
	pSensorResolution->SensorVideoWidth=OV2659_IMAGE_SENSOR_SVGA_WIDTH - 2 * 10;
	pSensorResolution->SensorVideoHeight=OV2659_IMAGE_SENSOR_SVGA_HEIGHT - 2 * 10;
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	{
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
			pSensorInfo->SensorPreviewResolutionX=OV2659_IMAGE_SENSOR_UVGA_WITDH - 2 * 10;
			pSensorInfo->SensorPreviewResolutionY=OV2659_IMAGE_SENSOR_UVGA_HEIGHT - 2 * 10;	
			break;
		default:
			pSensorInfo->SensorPreviewResolutionX=OV2659_IMAGE_SENSOR_SVGA_WIDTH - 2 * 10;
			pSensorInfo->SensorPreviewResolutionY=OV2659_IMAGE_SENSOR_SVGA_HEIGHT - 2 * 10;	
			break;
	}
	#else
		pSensorInfo->SensorPreviewResolutionX= OV2659_IMAGE_SENSOR_SVGA_WIDTH - 2 * 10;
		pSensorInfo->SensorPreviewResolutionY= OV2659_IMAGE_SENSOR_SVGA_HEIGHT - 2 * 20;
	#endif
	pSensorInfo->SensorFullResolutionX= OV2659_IMAGE_SENSOR_UVGA_WITDH - 2 * 10;
	pSensorInfo->SensorFullResolutionY= OV2659_IMAGE_SENSOR_UVGA_HEIGHT - 2 * 10;
...
UINT32 OV2659SetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	...
	OV2659SENSORDB("OV2659SetMaxFramerateByScenario: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
	...
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			OV2659SENSORDB("OV2659SetMaxFramerateByScenario MSDK_SCENARIO_ID_CAMERA_PREVIEW: lineLength = %d, dummy=%d\n",lineLength,dummyLine);
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			...
			frameHeight = (10 * pclk)/frameRate/lineLength;
			dummyLine = frameHeight - OV2659_PV_PERIOD_LINE_NUMS;
			OV2659SENSORDB("OV2659SetMaxFramerateByScenario MSDK_SCENARIO_ID_VIDEO_PREVIEW: lineLength = %d, dummy=%d\n",lineLength,dummyLine);			
			OV2659SetDummy(0, dummyLine);			
			break;			
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_CAMERA_ZSD:			
			...
			OV2659SENSORDB("OV2659SetMaxFramerateByScenario MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG: lineLength = %d, dummy=%d\n",lineLength,dummyLine);			
			OV2659SetDummy(0, dummyLine);			
			break;		
